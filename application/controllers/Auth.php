...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'mailtestphp96@gmail.com',
            'smtp_pass' => 'Pa$$w0rd_123',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('mailtestphp96@gmail.com', 'Test Programming');
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Account activation expired.</div>');
                    redirect('auth');
                }
            } else {
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email is not registered of activated!</div>');
                redirect('auth');
            }
        }
    }
    ...
    public function resetPassword()
    {
        ...
        if ($user) {
            ...
            if ($user_token) {
                // session ini hanya akan dilakukan ketika user sudah memasukan emailnya
